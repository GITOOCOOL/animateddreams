...
export const getAvailableLoras = async (): Promise<string[]> => {
  try {
    const response = await fetch(`${COMFY_HOST}/object_info`); // Fetch ALL object info to be safe
    if (!response.ok) throw new Error("Failed to fetch object info");
    
    const data = await response.json();
    console.log("ComfyUI Object Info Received. Scanning for LoRA nodes...");

    // Heuristic: Find any node that has an input named 'lora_name'
    // This is robust against node renaming (e.g. LoraLoader vs LoraLoaderModelOnly vs Custom Nodes)
    let foundLoras: string[] = [];
    const keys = Object.keys(data);
    
    for (const key of keys) {
        if (key.toLowerCase().includes("lora")) {
             console.log(`Inspecting potential LoRA node: ${key}`);
             const inputs = data[key]?.input?.required?.lora_name;
             if (inputs && Array.isArray(inputs) && Array.isArray(inputs[0])) {
                 console.log(`MATCH: Found LoRA list in node '${key}' with ${inputs[0].length} entries.`);
                 foundLoras = inputs[0];
                 break; // Found a valid list, stop searching
             }
        }
    }

    if (!foundLoras || foundLoras.length === 0) {
        console.warn("Scan complete. No non-empty LoRA lists found in any node.");
        // Last resort: Return a fake item so the user KNOWS the fetch happened but failed to find content
        return ["DEBUG_NO_LORAS_FOUND_ON_DISK"];
    }
    
    console.log("Final LoRA List:", foundLoras);
    return foundLoras;
  } catch (error) {
    console.error("Failed to fetch LoRAs:", error);
    return ["DEBUG_FETCH_ERROR"];
  }
};
 ...
 */
const modifyWorkflow = (baseWorkflow: any, prompt: string, settings?: ComfySettings, inputImageFilename?: string) => {
  ...
  if (newWorkflow["6"] && newWorkflow["6"].inputs) {
      newWorkflow["6"].inputs.text = `${prompt}, detailed face, realistic eyes, natural skin texture, masterpiece, best quality, 8k`;
  }
...
export const generateComfyImage = async (
  ...
  const workflow = modifyWorkflow(workflowTmpl, visualPrompt, settings, uploadedFilename);
  const clientId = crypto.randomUUID();

  return new Promise((resolve, reject) => {
    // WebSocket connection logic (Same as before)
    // ... (rest of the function is mostly unchanged, logic is inside the Promise)
    ...
    // Timeout removed by user request (Infinite wait)
  });
};
